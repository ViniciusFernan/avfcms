...
class AnuncioDAO extends Conn {
    ...
    public function getAnuncioPorId($id) {
        try{
            ...
            $select = new Select();
            $dadosAnuncio = $select->Select('*', 'anuncio', "idAnuncio=:id", "id={$id}");
    ...
    public function checarAnuncioSlugCadastrado($slugAnuncio, $idAnuncio){
        try{
            ...
            $select = new Select();
            $dadosAnuncio = $select->Select('*', 'anuncio', "slugAnuncio=:slugAnuncio {$where}", "slugAnuncio={$slugAnuncio}{$parse}");
            if($dadosAnuncio instanceof Exception) throw $dadosAnuncio;
    ...
    public function listarAnuncioPorUsuario($idUsuario){
        try{
            ...
            $select = new Select();
            $dadosAnuncio = $select->Select('*', 'anuncio', "idUsuario=:idUsuario", "idUsuario={$idUsuario}");
            if($dadosAnuncio instanceof Exception) throw $dadosAnuncio;
            if(!empty($dadosAnuncio)):
                return $dadosAnuncio;
            else:
